...
interface Model {
  ...
  model_name: string;
  trigger_word: string;
  training_id: string; // Updated to include training_id
}
...
export default function CreateImage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const output = await generateAIModelImage(
        prompt,
        model.model_name,
        model.training_id
      );
      // Assuming the output is a URL to the generated image
      setGeneratedImage(output[0]); // Adjust based on actual output structure
    } catch (error) {
