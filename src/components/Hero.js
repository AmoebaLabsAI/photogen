    ...
    return Array(numberOfColumns)
      ...
      .map(() => {
        ...
        do {
          ...
          randomDuration = Math.floor(
            Math.random() * (7087500 - 4725000 + 1) + 4725000
          );
          ...
          {columns.map((column, colIndex) => (
            ...
            >
              <div
                ...
                style={{ animationDuration: `${column.duration}ms` }}
              >
                {column.images.map((image, rowIndex) => (
                  <div
                    key={rowIndex}
                    className={`carousel-item ${
                      highlightedCell === `${colIndex}-${rowIndex}`
                        ? "highlighted"
                        : ""
                    }`}
                    onMouseEnter={() =>
                      setHighlightedCell(`${colIndex}-${rowIndex}`)
                    }
